...
describe('independent Desktop frame', () => {
  it('reserves a command bar for both framed modes and limits the inverted-L surface to extended mode', () => {
    ...
    try {
      ...
      expect(css).toMatch(/\[data-dsh-t3-surface="composer"\] \{[^}]*max-width: 770px;[^}]*border: 1px solid var\(--dshT3-border\);[^}]*border-radius: 14px;/)
      expect(css).toMatch(/\[data-dsh-t3-surface="composer-frame"\] \{[^}]*max-width: 780px;[^}]*margin-right: auto;[^}]*margin-left: auto;[^}]*padding: 4px;[^}]*border-radius: 22px;/)
      expect(appendChild).toHaveBeenCalledWith(style)
      dispose()
      expect(remove).toHaveBeenCalledOnce()
    } finally {
