...
describe('independent Desktop frame', () => {
  it('reserves a command bar for both framed modes and limits the inverted-L surface to extended mode', () => {
    ...
    try {
      ...
      expect(css).toMatch(/body\[data-dsh-desktop-mode="compatibility"\]\[data-dsh-desktop-material="off"\] \{[^}]*--dsh-desktop-frame-fill: var\(--dsw-alias-bg-base\);/)
      expect(css).toMatch(/#root div\[style\*="grid-template-columns"\]:has\(\[data-shell-overlay\]\) \{[^}]*background: var\(--dsw-alias-bg-base\);/)
      ...
      expect(css).toMatch(/\[data-dsh-t3-surface="composer"\] \{[^}]*max-width: 748px;[^}]*border: 1px solid var\(--dshT3-border\);[^}]*border-radius: 17px;/)
      expect(css).toMatch(/\[data-dsh-t3-surface="composer-frame"\] \{[^}]*max-width: 758px;[^}]*margin-right: auto;[^}]*margin-bottom: 8px;[^}]*margin-left: auto;[^}]*padding: 4px;[^}]*border-radius: 22px;/)
      expect(css).toMatch(/\[data-dsh-t3-surface="composer-seat"\] \{[^}]*background-image: linear-gradient\(to top, var\(--dsw-alias-bg-base\) 30px, transparent 40px\);/)
      expect(css).toMatch(/button\[aria-label="好的回答"\],\s*body\[data-dsh-desktop-mode="compatibility"\]\[data-dsh-desktop-material="off"\] #root div\[style\*="grid-template-columns"\]:has\(\[data-shell-overlay\]\) button\[aria-label="有问题的回答"\] \{[^}]*display: none;/)
      expect(css).toMatch(/\[class\$="_timeStart"\] \{[^}]*opacity: 1 !important;[^}]*order: 1;[^}]*padding-left: 12px;[^}]*padding-right: 0;/)
      expect(css).toMatch(/\[class\$="_timeEnd"\] \{[^}]*opacity: 1 !important;[^}]*order: -1;[^}]*padding-left: 6px;[^}]*padding-right: 12px;/)
      expect(css).toMatch(/button\[aria-label="在新对话中分支"\] \{[^}]*opacity: 0;[^}]*pointer-events: none;/)
      expect(css).toMatch(/\[data-time-hover-root\]:hover button\[aria-label="复制"\][^{]*\{[^}]*opacity: 1;[^}]*pointer-events: auto;/)
      expect(css).toMatch(/\[data-ds-dark-theme\]\[data-dsh-desktop-mode="compatibility"\]\[data-dsh-desktop-material="off"\] \[data-dsh-t3-surface="composer"\] \{[^}]*background: color-mix\(in srgb, var\(--dsw-alias-bg-base\) 96%, #fff 4%\);[^}]*box-shadow: 0 1px 2px color-mix\(in srgb, #000 15%, transparent\), 0 4px 14px color-mix\(in srgb, #000 20%, transparent\);/)
      expect(css).toMatch(/\[data-ds-dark-theme\]\[data-dsh-desktop-mode="compatibility"\]\[data-dsh-desktop-material="off"\] \[data-dsh-t3-surface="composer-frame"\] \{[^}]*background: rgba\(255, 255, 255, 0.02\);/)
      expect(appendChild).toHaveBeenCalledWith(style)
      dispose()
      expect(remove).toHaveBeenCalledOnce()
    } finally {
