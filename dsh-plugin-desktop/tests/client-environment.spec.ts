...
describe('independent Desktop frame', () => {
  it('reserves a command bar for both framed modes and limits the inverted-L surface to extended mode', () => {
    ...
    try {
      ...
      expect(css).toMatch(/\[data-dsh-t3-surface="composer"\] \{[^}]*max-width: 738px;[^}]*border: 1px solid var\(--dshT3-border\);[^}]*border-radius: 14px;/)
      expect(css).toMatch(/\[data-dsh-t3-surface="composer-frame"\] \{[^}]*max-width: 748px;[^}]*margin-right: auto;[^}]*margin-left: 64px;[^}]*padding: 4px;[^}]*border-radius: 22px;/)
      expect(css).not.toMatch(/> div:nth-child\(2\) > div:first-child > div:first-child \{/)
      expect(appendChild).toHaveBeenCalledWith(style)
      dispose()
      expect(remove).toHaveBeenCalledOnce()
    } finally {
