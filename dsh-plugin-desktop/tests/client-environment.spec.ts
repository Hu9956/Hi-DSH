...
describe('independent Desktop frame', () => {
  it('reserves a command bar for both framed modes and limits the inverted-L surface to extended mode', () => {
    ...
    try {
      ...
      expect(css).toMatch(/\[data-dsh-t3-surface="composer"\] \{[^}]*max-width: 748px;[^}]*border: 1px solid transparent;[^}]*border-radius: 17px;[^}]*box-shadow: none;/)
      expect(css).toMatch(/\[data-dsh-t3-surface="composer-frame"\] \{[^}]*max-width: 758px;[^}]*margin-right: auto;[^}]*margin-bottom: 8px;[^}]*margin-left: auto;[^}]*padding: 4px;[^}]*border-radius: 22px;[^}]*background: rgba\(229, 233, 239, 0.7\);/)
      expect(css).toMatch(/\[data-ds-dark-theme\]\[data-dsh-desktop-mode="compatibility"\]\[data-dsh-desktop-material="off"\] \[data-dsh-t3-surface="composer-frame"\] \{[^}]*background: rgba\(238, 243, 245, 0.10\);/)
